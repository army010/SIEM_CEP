...
public class SymVirusSubscriber implements StatementSubscriber {
    ...
    public String getStatement() {
     String logComplexQuery = "select * from ViruslogEvent((type = 'file' AND (action = 'connection not terminated' OR action = 'not deleted') AND information LIKE 'threat was detected %')) having count(*) > 0";
       // String logComplexQuery = "select * from ViruslogEvent((type = 'file' AND action = 'not deleted' AND information LIKE 'threat was detected %')) having count(*) > 0";
  ...
  //  String logComplexQuery = "select * from ViruslogEvent.win:expr_batch(current_count >= 1)";
    ...
    public void update(Map<String, ViruslogEvent> eventMap) {
